 ...
 */
public class PythonJavaCfgMainGenerator extends CfgMainGenerator {
	
	private static String generateInitialValue(Configuration cfg, Instance i, Property p, Expression e, Context ctx) {
        StringBuilder tempbuilder = new StringBuilder();
        if (e == null) {
        	tempbuilder.append("(" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx) + ")"); //we should explicitly cast default value, as e.g. 0 is interpreted as an int, causing some lossy conversion error when it should be assigned to a short
        	tempbuilder.append(PythonJavaHelper.getDefaultValue(p.getTypeRef().getType()));
        } else {
            tempbuilder.append("(" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx) + ") ");
            tempbuilder.append("(");
    ...
    public static void generateInstances(Configuration cfg, Context ctx, StringBuilder builder) {
        ...
        for (Instance i : ConfigurationHelper.allInstances(cfg)) {
            ...
            for (Property a : ConfigurationHelper.allArrays(cfg, i)) {
                builder.append("final " + PythonJavaHelper.getJavaType(a.getTypeRef().getType(), true, ctx) + " " + i.getName() + "_" + a.getName() + "_array = new " + PythonJavaHelper.getJavaType(a.getTypeRef().getType(), false, ctx) + "[");
    ...
    public void generateMainAndInit(Configuration cfg, ThingMLModel model, Context ctx) {
        ...
        PythonJavaHelper.generateHeader(pack, pack, builder, ctx, true, api);
        if (gui) {
