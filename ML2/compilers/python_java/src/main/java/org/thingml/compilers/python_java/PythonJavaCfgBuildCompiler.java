 ...
 */
public class PythonJavaCfgBuildCompiler extends CfgBuildCompiler {    
	...
	protected void doGenerate(Configuration cfg, Context ctx, String pomResource) throws Exception {
        ...
        for (String src : PythonJavaHelper.allSrcFolders(cfg)) {
        ...
        for (String dep : PythonJavaHelper.allMavenDep(cfg)) {
            pom = pom.replace("<!--DEP-->", "<!--DEP-->\n" + dep);
        }

        for (String repo : PythonJavaHelper.allMavenRepo(cfg)) {
