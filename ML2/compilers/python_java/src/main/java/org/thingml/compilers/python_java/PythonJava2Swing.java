 ...
 */
public class PythonJava2Swing extends CfgExternalConnectorCompiler {
            ...
            for (Message msg : entry.getValue()) {
                ...
                for (Parameter p : msg.getParameters()) {
                    ...
                    if (p.getTypeRef().getType() instanceof Enumeration) {
                        ...
                        tempBuilder.append("field" + msg.getName() + "_via_" + port.getName() + "_" + ctx.firstToUpper(p.getName()) + ".setText(\"" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null, ctx) + "\");\n");
                    }
                ...
                for (Parameter p : msg.getParameters()) {
                    ...
                    if (p.getTypeRef().getCardinality() == null) {
                        if (p.getTypeRef().getType() instanceof Enumeration) {
                            ...
                            tempBuilder.append("StringHelper.toObject (" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx) + ".class, getField" + msg.getName() + "_via_" + port.getName() + "_" + ctx.firstToUpper(p.getName()) + "().getText())");
                            parseBuilder.append("StringHelper.toObject (" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx) + ".class, params[" + msg.getParameters().indexOf(p) + "].trim())");
                        }
                    } else {
                ...
                for (Parameter p : msg.getParameters()) {
                    ...
                    if (p.getTypeRef().getCardinality() == null) {
                        if (p.getTypeRef().getType() instanceof Enumeration) {
                            ...
                            tempBuilder.append("(");
                            if (PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx).equals("int"))
                                tempBuilder.append("Integer");
                            else
                                tempBuilder.append(ctx.firstToUpper(PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx)));
                            tempBuilder.append(")StringHelper.toObject (" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), false, ctx) + ".class, getField" + msg.getName() + "_via_" + port.getName() + "_" + ctx.firstToUpper(p.getName()) + "().getText())");
                        }
                    } else {
