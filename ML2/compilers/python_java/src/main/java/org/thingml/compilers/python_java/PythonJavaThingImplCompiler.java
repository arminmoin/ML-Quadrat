 ...
 */
public class PythonJavaThingImplCompiler extends FSMBasedThingImplCompiler {
	...
	public void generateMessages(Message m, Context ctx) {
		...
		PythonJavaHelper.generateHeader(pack, rootPack, builder, ctx, false, false);
		...
		for (Parameter p : m.getParameters()) {
			if (m.getParameters().indexOf(p) > 0)
				builder.append(", ");
			builder.append("final " + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx)
		...
		for (Parameter p : m.getParameters()) {
			...
			if (p.getTypeRef().isIsArray() || p.getTypeRef().getCardinality() != null) {
				cast = PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null, ctx);
			} else {
				if (PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null, ctx)
						.equals("int"))
					cast = "Integer";
				else if (PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null, ctx)
						.equals("char"))
					cast = "Character";
				else if (PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null, ctx)
						.contains("."))
					cast = PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().getCardinality() != null,
							ctx); // extern datatype with full qualified name
				else
					cast = ctx.firstToUpper(PythonJavaHelper.getJavaType(p.getTypeRef().getType(),
		...
		for (Parameter p : m.getParameters()) {
			builder.append(
					"public final " + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx)
							+ " " + ctx.protectKeyword(p.getName()) + ";\n");
		}
		...
		for (Parameter p : m.getParameters()) {
			builder.append(
					", final " + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx) + " "
							+ ctx.protectKeyword(p.getName()));
		}
		builder.append(") {\n");
	...
	protected void generateFunction(Function f, Thing thing, StringBuilder builder, Context ctx) {
		final String returnType = PythonJavaHelper.getJavaType(((f.getTypeRef() != null) ? f.getTypeRef().getType() : null),
		...
		PythonJavaHelper.generateParameter(f, builder, ctx);
		builder.append(") {\n");
	...
	public void generateImplementation(Thing thing, Context ctx) {
		...
		PythonJavaHelper.generateHeader(pack, pack, builder, ctx, false, hasMessages);
		...
		for (Port p : ThingMLHelpers.allPorts(thing)) {
			if (!AnnotatedElementHelper.isDefined(p, "public", "false")) {
				for (Message m : p.getReceives()) {
					...
					PythonJavaHelper.generateParameter(m, builder, ctx);
					builder.append("){\n");
		...
		for (Property p : ThingHelper.allPropertiesInDepth(thing)) {
			...
			builder.append(PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx) + " " + ctx.getVariableName(p));
			builder.append(";\n");
		}
		for (Property p : ThingHelper.allSessionsProperties(thing)) {
			...
			builder.append(";\n");
			builder.append("private " + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx)
			...
			if (!p.isReadonly()) {
				builder.append("private void set" + ctx.firstToUpper(ctx.getVariableName(p)) + "("
						+ PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx) + " "
		...
		for (Property p : ThingHelper.allPropertiesInDepth(thing)) {
			builder.append("public " + PythonJavaHelper.getJavaType(p.getTypeRef().getType(), p.getTypeRef().isIsArray(), ctx)
					+ " get" + ctx.firstToUpper(ctx.getVariableName(p)) + "() {\nreturn " + ctx.getVariableName(p)
					+ ";\n}\n\n");
			if (!p.isReadonly()) {
			...
			if (generateEntry) {
				...
				if (resetProperties) {
					...
					for(Property p : cs.getProperties()) {
						...
						builder.append("(" + PythonJavaHelper.getJavaType(p.getTypeRef().getType(),
								p.getTypeRef().getCardinality() != null, ctx) + ")");
						if (p.getInit() != null) {
							ctx.getCompiler().getThingActionCompiler().generate(p.getInit(), builder, ctx);
						} else {
							builder.append(PythonJavaHelper.getDefaultValue(p.getTypeRef().getType()));
						}
						builder.append(");\n");
					}
				}
